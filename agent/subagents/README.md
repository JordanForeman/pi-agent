...
Direct `skills:` are a narrow complete default for that role; do not use YAML array syntax. A `skill` supplied by a subagent call overrides role defaults, so callers must pass the complete desired skill list. Ordinary child roles leave fanout and follow-up delegation to the parent unless they explicitly declare a supported `subagent` tool.
