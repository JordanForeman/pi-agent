...
- A routine commit and push of a topic branch is the final step of the work, not a separate request — carry a directed change through to pushed without stopping to confirm. This does not extend to force-push, rewriting published history, or pushing to a shared or protected branch; those stay under `safety` and the confirmation rule above.
- Existence is not state. A branch, a commit, or an open PR proves the artifact was created, not that it landed. Before reporting work as pushed, merged, or shipped, re-fetch and confirm the commit is an ancestor of the remote branch and the change is present where you claim it is. A commit stranded on a branch whose PR already merged looks identical to one that shipped.
