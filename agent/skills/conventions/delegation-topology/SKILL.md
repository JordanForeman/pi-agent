...
- Verify a named specialist is discoverable in the active profile before building a batch around it. Agent definitions are resolved per profile, so one that exists on disk for another profile is simply absent here — and the failure surfaces at dispatch, after you have planned the fan-out. When it is missing, inline its instructions into a generic child instead.
